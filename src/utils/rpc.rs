...
pub fn get_token_data(token_address: &Vec<u8>) -> (String, String, String) {
    ...
    let responses = batch_calls
        .add(
            abi::abi::erc20::functions::Name {},
            contract_address.clone(),
        )
        .add(
            abi::abi::erc20::functions::Symbol {},
            contract_address.clone(),
        )
        .add(
            abi::abi::erc20::functions::Decimals {},
            contract_address.clone(),
        )
        .execute()
        .unwrap()
    ...
    let name = match RpcBatch::decode::<_, abi::abi::erc20::functions::Name>(&responses[0]) {
        Some(contract_name) => contract_name.to_string(),
        None => "Name Call Reverted".to_string(),
    };
    let symbol = match RpcBatch::decode::<_, abi::abi::erc20::functions::Symbol>(&responses[1]) {
        Some(contract_symbol) => contract_symbol.to_string(),
        None => "Name Call Reverted".to_string(),
    };
    let decimals = match RpcBatch::decode::<_, abi::abi::erc20::functions::Decimals>(&responses[2])
    {
